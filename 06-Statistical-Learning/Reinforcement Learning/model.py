...
class Walk(Environment):
    ...
    Wind = {}  # move probabilities due to wind (default is no wind)
    ...
    def display(self, states=None, marker='o', marker_size=250, color_dict={'+': 'g', '-': 'r', '0': 'b'}, interval=200):
        """Display the states."""        
    ...
    def display_values(values):
        ...
        plt.figure(figsize=np.array(Walk.Size) / 2)
...
class Maze(Environment):
    ...
    def display_values(values):
        """Display the value function."""
        plt.figure(figsize=np.array(Maze.Map.shape) / 2)
...
class Game(Environment):
    ...
    def __init__(self, adversary_policy='random', player=1, play_first=True):
        ...
        self.adversary = BaseAgent(self, adversary_policy, player=-player)
        self.player = player
    ...
    def is_terminal(self, state):
        """Check whether a state is terminal."""
        player, board = state
        return bool(self.get_reward(state)) or bool(board.astype(bool).all())
    ...
    def get_transition(self, state, action):
        ...
        if not self.is_terminal(state):
            player, board = state
            if action == "pass" and player == -self.player:
...
class TicTacToe(Game):
    ...
    def get_available_actions(self, state):
        ...
        if not self.is_terminal(state):
            ...
            actions = [(int(x), int(y)) for x, y in zip(x_, y_)]
        return actions
    ...
    def display(self, states=None, marker1='X', marker2='o', marker_size=1000, color1='b', color2='r', interval=300):
        """Display states."""
...
class Nim(Game):
    ...
    def get_available_actions(self, state):
        ...
        if not self.is_terminal(state):
            ...
            actions = [(int(row), number + 1) for row in rows for number in range(board[row])]
        return actions
    ...
    def display(self, states=None, marker='o', marker_size=300, color='gold', color_dict={1: 'b', -1: 'r'}, interval=200):
        """Display states."""
        board = np.array(Nim.Board_Size).astype(int)
        width = max(Nim.Board_Size)
        ...
        if states is not None:
            ...
            for player, board in states:
                ...
                for row in np.where(board)[0]:
                    for col in range(board[row]):
                        ...
                        y.append(col)
                # player
                marker_color.append(len(x) * [color] + [color_dict[player]]) 
                x.append(0)
                y.append(width)
            ...
            for row in np.where(board)[0]:
                for col in range(board[row]):
                    ...
                    y.append(col)
            # player
            marker_color = len(x) * [color] + [color_dict[player]]
            x.append(0)
            y.append(width)
            position = (x, y)
...
class ConnectFour(Game):
    ...
    def display(self, states=None, marker1='o', marker2='o', marker_size=500, colors=['gold', 'r'], interval=200):
        """Display states."""
